...
- `ferrisfetch-0.8.6.ebuild`: Portage ebuild using `EAPI=8`, `cargo.eclass`, and `shell-completion.eclass`.
...
2. Copy the ebuild and metadata files:
   ```bash
   cp ferrisfetch-0.8.6.ebuild metadata.xml /var/db/repos/localrepo/app-misc/ferrisfetch/
   ```

3. Generate the Manifest file:
   ```bash
   cd /var/db/repos/localrepo/app-misc/ferrisfetch
   ebuild ferrisfetch-0.8.6.ebuild manifest
   ```

4. Test compilation and installation:
   ```bash
   ebuild ferrisfetch-0.8.6.ebuild clean compile
   ebuild ferrisfetch-0.8.6.ebuild install
   ```
...
2. Commit conforming to Gentoo git conventions:
   ...
   git commit -m "app-misc/ferrisfetch: new package, add 0.8.6"
   ```
