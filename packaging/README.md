...
| **Gentoo Linux** | [`gentoo/ferrisfetch-0.8.5.ebuild`](gentoo/ferrisfetch-0.8.5.ebuild) | `ebuild` / `emerge` | Portage ebuild |
...
4. Commit and push:
   ...
   git commit -m "Update to version 0.8.5"
...
cargo deb
```
Output: `target/debian/ferrisfetch_0.8.5-1_amd64.deb`
...
2. Run lintian to verify compliance:
   ```bash
   lintian --pedantic -I ferrisfetch_0.8.5-1.dsc
   ```
3. Upload to Debian Mentors:
   ```bash
   dput mentors ferrisfetch_0.8.5-1_source.changes
   ```
...
cargo generate-rpm
```
Output: `target/generate-rpm/ferrisfetch-0.8.5-1.x86_64.rpm`
...
2. Build in your Copr repository:
   ```bash
   copr-cli build kk376/ferrisfetch ~/rpmbuild/SRPMS/ferrisfetch-0.8.5-1.*.src.rpm
   ```
...
- **Source files**: [`packaging/gentoo/ferrisfetch-0.8.5.ebuild`](gentoo/ferrisfetch-0.8.5.ebuild)

#### Local Build with ebuild
```bash
ebuild packaging/gentoo/ferrisfetch-0.8.5.ebuild digest
ebuild packaging/gentoo/ferrisfetch-0.8.5.ebuild clean compile install
```

#### Gentoo Overlay / Main Repository Submission
1. Test with `pkgcheck`:
   ```bash
   pkgcheck scan ferrisfetch-0.8.5.ebuild
   ```
...
4. Commit with message `ferrisfetch: new package at 0.8.5`.
...
1. Tag the release: `git tag -a v0.8.5 -m "Release v0.8.5"` and push tags to GitHub.
2. Generate source archive checksum:
   ```bash
   curl -sL https://github.com/kk376/ferrisfetch/archive/refs/tags/v0.8.5.tar.gz | sha256sum
   ```
