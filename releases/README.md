# FerrisFetch Releases (v0.1.0)

Official pre-built release binaries, native OS distribution packages, source archives, and build recipes for **FerrisFetch v0.1.0** — a fast, lightweight Linux system information fetch tool written in Rust.
...
| :--- | :--- | :--- |
| `ferrisfetch-0.1.0-source.tar.gz` | `.tar.gz` | Clean source code distribution tarball (v0.1.0) |
| `ferrisfetch-0.1.0-source.zip` | `.zip` | Clean source code distribution zip archive (v0.1.0) |
...
| `ferrisfetch-0.1.0-fedora-x86_64.tar.gz` | Fedora / RHEL (`x86_64`) | Binary release bundle containing executable, documentation, and spec |
...
```bash
# Using RPM directly:
sudo rpm -Uvh ferrisfetch-0.1.0-1.x86_64.rpm

# Or using DNF:
sudo dnf install ./ferrisfetch-0.1.0-1.x86_64.rpm
ferrisfetch
```
